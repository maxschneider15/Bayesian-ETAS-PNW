...
// [[Rcpp::export]]
double makeASum(std::vector<double> &vecvec) {
    double sumout;
    sumout = std::accumulate(vecvec.begin(), vecvec.end(), 0.0);
    return sumout;
  }
...
// extern "C" double MatrixFn(double x1, double y1, double x2, double y2, 
...
// [[Rcpp::export]]
double MatrixFn(double x1, double y1, double x2, double y2) {
    Rcout <<  x1 << " " << x2 << endl;
    Rcout <<  y1 << " " << y2 << endl;
    double x = x1 - x2; //calculating number to square in next step
    double y = y1 - y2;
...
// param values
// [[Rcpp::export]]
void STsampleBranchingMSTest(std::vector<double> &ts, std::vector<double> &lons, std::vector<double> &lats, 
                         std::vector<double> &marks, double M0, double maxT, 
                         double mu, double logK, double alpha, double c, double p, double d, double q,  
                         NumericMatrix &cat_Gevals_grid,
                         NumericVector branching) {
  // Initialize some objects.
  int n = ts.size();
  int parent;
  double temp;
  ...
  for (int i = 1; i < n; i++) {
    ...
    for (int j = 0; j < i; j++) { //(GR: check this iteratures through...)
      ...
      // temp = kappaevals[j]*pow(ts[i]-ts[j]+c,-p);
      // Unnormed Omori using squared distance (correct)
      temp = kappaevals[j]*pow(ts[i]-ts[j]+c,-p)*pow( sqDistanceCalculate(lons[i], lats[i], lons[j], lats[j]) + d,-q); 
      // Unnormed Omori using distance (incorrect)
      // temp = kappaevals[j]*pow(ts[i]-ts[j]+c,-p)*pow( distanceCalculate(lons[i], lats[i], lons[j], lats[j]) + d,-q); 
      // temp = kappaevals[j]*(p-1)*pow(c, p-1)*pow(ts[i]-ts[j]+c,-p)*pow( sqDistanceCalculate(lons[i], lats[i], lons[j], lats[j])  +d , -q); // ITO, Ross (21)
      probs.push_back(temp);
...
// [[Rcpp::export]]
std::vector<int> sampleBranchingProbs(std::vector<double> &ts, std::vector<double> &marks, 
                                      double M0,  double mu, double logK, double alpha, double c, 
                                      double p, std::vector<int> &branching) {
  // Initialize some objects.
  int n = ts.size();
  int parent;
  double temp;
  ...
  // Initialize branching vector. Clear the inputted values, make the vector size n and add an element "0"
  branching.clear();
  branching.reserve(n);
  branching.push_back(0);
...
// [[Rcpp::export]]
// with or without & in argument?
double interpolateGevals(double this_d, double this_q, NumericMatrix &Gevals_grid, int index){
  // Perform 2D linear interpolation between nearest d/q elements to approximate the Geval for this_d/this_q
  // sizeof array / sizeof array[0] is the best way to get nrow of an array
  // https://stackoverflow.com/questions/10274162/how-to-find-2d-array-size-in-c/24984668
  // int num_rows = sizeof(Gevals_grid)/sizeof(Gevals_grid[0]);
  ...
  // int num_rows = Gevals_grid.size();
  // int num_cols = Gevals_grid[0].size();
  // int last_col = num_cols;
  int num_rows = Gevals_grid.nrow();
  int col_eqk = index;
  // std::cout << "Number rows/cols: " << num_rows << " / " << last_col << "\n";

  // Pull out the d and q values in this grid 
  std::vector<double> grid_ds (num_rows);
  std::vector<double> grid_qs (num_rows);
  // for (int i = 0; i < num_rows; i++) {
  //   grid_ds[i] = Gevals_grid[0][i];
  //   grid_qs[i] = Gevals_grid[1][i];
  // }
  for (int i = 0; i < num_rows; i++) {
    grid_ds[i] = Gevals_grid(i, 0);
    grid_qs[i] = Gevals_grid(i, 1);
  ...
  // Find min/max values
  double min_d = *std::min_element(grid_ds.begin(), grid_ds.end());  
  double max_d = *std::max_element(grid_ds.begin(), grid_ds.end());  
  double min_q = *std::min_element(grid_qs.begin(), grid_qs.end());  
  double max_q = *std::max_element(grid_qs.begin(), grid_qs.end());  
  
  if(this_d < min_d){
    this_d = min_d+0.001;
    std::cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Changed to " << this_d << "\n";
  ...
  // x variable for d, y variable for q
  // double this_interp_Geval =
  //   BilinearInterpolation(q11=Gevals_grid[r_d_below_q_below][col_eqk], q12=Gevals_grid[r_d_above_q_below][col_eqk], 
  //                         q21=Gevals_grid[r_d_below_q_above][col_eqk], q22=Gevals_grid[r_d_above_q_above][col_eqk], 
  //                                                                                                        x1=*d_below, x2=*d_above,
  //                                                                                                        y1=*q_below, y2=*q_above, x=this_d, y=this_q);
  double this_interp_Geval = BilinearInterpolation(q11=Gevals_grid(r_d_below_q_below,col_eqk), q12=Gevals_grid(r_d_above_q_below,col_eqk),
...
// // TODO: Where is the log(prior)? Since it's a Unif prior, this = 0
// // Inputs: eqk catalog,
// //    z: time between each triggered event and its triggering event
// //    kappaevals: computed in sampleBranching fn
// //    Gevals: input by user
// //    given values for c and p and maxT
// // Outputs: log marginal posterior value for (c,p), based on the given values
// double SThBranchingPosteriorMS(std::vector<double> &ts, std::vector<double> &marks, 
//                                std::vector<double> &z,  std::vector<double> &Hevals,
//                                std::vector<double> &kappaevals, std::vector<double> &Gevals, double c, double p) {
//   
//   // If c or p values extreme, break func with error code
...
//     // Add to the log posterior the log of h(time diff_i) (second product in eqn in #4, pg 5)
//     // logpost += log(p-1) + (p-1)*log(c) - p*log(z[i]+c); // also from Ross (21)
//     
...
//     // Calculate the first product in eqn in #4, pg 5. First calculate the H term (=temp)
//     // and then multiply by kappa for eqk i. Repeat over all eqks to get the sum.
//     // The sum is subtracted from the log posterior, so we can equivalently subtract each eqk's 
//   // contribution, looping over all eqks
//   // temp = 1 - (pow(c,p-1) / pow(maxT-ts[i]+c,p-1)); // H(T-t_i). from the online article, but changed in Ross (21)
//     
//   // 6.11.21. Unnormalized O
//   // temp = (pow(maxT-ts[i] +c, 1-p) - pow(c, 1-p)) / (1-p); // H(T-t_i)
//   
//   logpost = logpost - kappaevals[i]*Gevals[i]*Hevals[i]; 
//   }
//   
// 
//   
...
double SThBranchingPosteriorMS(std::vector<double> &ts, std::vector<double> &marks, 
  ...
  for (int i = 0; i < z.size(); i++){
    hSums.push_back( -1 * p*log(z[i]+c) ); // Unnormalized O
  }
...
// [[Rcpp::export]]
double STgBranchingPosteriorMS(std::vector<double> &lons, std::vector<double> &lats, 
  ...
  if (d <= 0 || q <= 1 || d >= 10 || q >= 10) {
    return(-9999999);
  }
  
  // for cats D
  // if (d <= 0.025 || q <= 1.05 || d >= 0.175 || q >= 2.25) {
  //   return(-9999999);
  // }
  // for cats A
  // if (d <= 0.3 || q <= 1.25 || d >= 1.75 || q >= 2.75) {
  //   return(-9999999);
  // }
  // for cats B
  // if (d <= 0.3 || q <= 1.05 || d >= 1.75 || q >= 2.25) {
  //   return(-9999999);
  // }
  ...
  //   q = 8;
  // }
  // Rcout << "dists1 " <<  dists[0] << ", dists2 " << dists[1] << " dists5 " <<  dists[4] << " dists10 " <<  dists[10] << endl;
...
double STkappaBranchingPosteriorMS(std::vector<double> &ts, std::vector<double> &marks, 
  ...
  // Uniform priors on K/alpha
  if (alpha < 0 || alpha > 10 || K < 0 || K > 10) {
    return(-9999999);
  }
...
// [[Rcpp::export]]
void STestimateETASBranchingInteractionMS(std::vector<double> &ts, std::vector<double> &marks,
                                          std::vector<double> &lons, std::vector<double> &lats, 
                                          std::vector<int> &branching, double maxT, double M0, 
                                          int sims, int numMCMCSamples, double mu, double logK,
                                          double alpha, double c, double p, double d, double q, 
                                          std::vector<double> & mus,
                                          std::vector<double> & logKs, std::vector<double> & alphas,
                                          std::vector<double> & cs, std::vector<double> & ps, 
                                          std::vector<double> & ds, std::vector<double> & qs) {  
  ...
  double mualpha = 0.1, mubeta = 0.1; //prior params - change for mu_ST?
  ...
  std::normal_distribution<> rnorm_K(0, 0.15);
  ...
  for (int s=0; s<sims; s++) {
    ...
    // Run the sampleBranching fn with the current param values
    // STsampleBranchingMS(ts, lons, lats, marks, M0,mu,logK,alpha,c,p, d,q,branching);
    STsampleBranchingMS(ts, lons, lats, marks, M0,mu,logK,alpha,c,p, d,q);
    
    // TODO: Can we remove this z.clear?
    z.clear();
    dists.clear();
    // Initialize vector to put the index of each eqk's triggering eqk
    ...
    for (int i=0; i<n; i++) {
      ...
      if (branching[i] > 0) {
        ...
        dists.push_back(sqDistanceCalculate(lons[i], lats[i], lons[branching[i]-1], lats[branching[i]-1]));
    ...
    std::gamma_distribution<> rgamma(mualpha+numbackground, 1/(mubeta+maxT));
    mu = rgamma(gen)/(500^2);
    mus.push_back(mu);
    // Rcout <<  "mus are " << mus[mus.size()-1] << endl;
    ...
    for (int i=0; i < numMCMCSamples; i++) {
      ...
      // Rcout <<  "new K alpha posteriornew K alpha posteriornew K alpha posteriornew K alpha posteriornew K alpha posteriornew K alpha posteriornew K alpha posterior" << endl;
    ...
    // Clear kappaevals (MS: not yet filled - can I cut this line?)
    kappaevals.clear();
    K = exp(logK);
    // For each eqk...
    for (int i=0; i<n; i++) {
    ...
    // currposterior = SThBranchingPosteriorMS(ts,marks,z, kappaevals, Hevals, Gevals, c,p);
    ...
    for (int i=0; i < numMCMCSamples; i++) {
      ...
      // Rcout << "Proposal posterior: " <<  newposterior << ", comparison to old posterior: " << exp(newposterior-currposterior) << endl;
      ...
      if (runif(gen) < exp(newposterior-currposterior)) {
        // Rcout << "Proposal posterior: " <<  newposterior << endl;
        c = newc;
        p = newp;
        currposterior = newposterior;
      }
    ...
    for (int i=0; i < numMCMCSamples; i++) {
      ...
      // if(newd < 0.8){
      //   newd = 0.8;
      //   // Rcout <<  "hey! d small" << endl;
      // }
      // if(newd > 1.2){
      //   newd = 1.2;
      //   // Rcout <<  "hey! d big" << endl;
      // }
      // if(newq < 1.8){
      //   newq = 1.8;
      //   // Rcout <<  "hey! q small" << endl;
      // }
      // if(newq > 2.2){
      //   newq = 2.2;
      //   // Rcout <<  "hey! q big" << endl;
      // }
      // Generate a value from the posterior distn using the proposal c and p
  ...
  // std::copy(vmus,vmus+sims,mus.begin());
  // std::copy(vlogKs,vlogKs+sims,*logKs.begin());
  // std::copy(valphas,valphas+sims,*alphas.begin());
  // std::copy(vcs,vcs+sims,*cs.begin());
  // std::copy(vps,vps+sims,ps.begin());
  // std::copy(vds,vds+sims,ds.begin());
  // std::copy(vqs,vqs+sims,qs.begin());
  ...
  // std::copy(vds.begin(),vds.end(),ds);
  // std::copy(vqs.begin(),vqs.end(),qs);
  // return;
}
...
// [[Rcpp::export]]
// double spatialArea,
// std::vector<int> Starget,
List STestimateETASBranchingFreeBMSList(std::vector<double> &ts, std::vector<double> &marks,
                                       std::vector<double> &lons, std::vector<double> &lats, 
                                       std::vector<int> &branching, double maxT, double M0, 
  ...
  for (int s=0; s<sims; s++) {
    ...
    // Get the posterior value for p(c, p) using the current c and p
    // currposterior = SThBranchingPosteriorMS(ts,marks,z,maxT,kappaevals, Gevals, c,p);
    posteriorcpCurr = SThBranchingPosteriorMS(ts,marks,z,maxT,kappaevals, Gevals, cCurr, pCurr);
    // Rcout << "Starting c and p " <<  c << "  " << p  << endl;
    // For each MCMC sample...
    for (int i=0; i < numMCMCSamples; i++) {
      // Rcout << "MCMC sample: " << i << endl;
      ...
      posteriorcpNew = SThBranchingPosteriorMS(ts, marks, z, maxT, kappaevals, Gevals, cNew, pNew);

      // Rcout << "Proposal posterior: " <<  newposterior <<  "Current posterior: " <<  currposterior << ", comparison to old posterior: " << exp(newposterior-currposterior) << endl;
      // newposterior = hBranchingPosteriorMH(ts,marks,z,maxT,kappaevals,newc,newp);
      // Metropolis-Hastings step.
      // Generate a random unif number and accept the proposal c and p and posterior if it
      // is less than the ratio of the new posterior and current posterior
    ...
    // currposterior = STgBranchingPosteriorMS(lons, lats, marks, dists, maxT, kappaevals, Hevals, cat_Gevals_grid, d, q);
    // currposterior = STgBranchingPosteriorMS(lons, lats, marks, dists, maxT, kappaevals, Hevals, Gevals, d, q);
    posteriordqCurr = STgBranchingPosteriorMS(lons, lats, marks, dists, maxT, kappaevals, Hevals, Gevals, dCurr, qCurr);
    // // currposterior = STgBranchingPosteriorMS(lons, lats, marks, dists, maxT, kappaevals, Hevals, d, q);
    // // Rcout << "Current posterior: " <<  currposterior << endl;
    //
    // // Rcout <<  "currposterior d/q posterior currposterior d/q posterior" << currposterior << endl;
    ...
    for (int i=0; i < numMCMCSamples; i++) {
      ...
      posteriordqNew = STgBranchingPosteriorMS(lons, lats, marks, dists, maxT, kappaevals, Hevals, Gevals, dNew, qNew);
      // Rcout <<  "new d/q posterior new d/q posterior" << newposterior << endl;
      // newposterior = STgBranchingPosteriorMS(lons, lats, marks, dists, maxT, kappaevals, Hevals, newd, newq);
      // Rcout << "mu: " <<  mu << ", K: " <<  K << ", alpha: " <<  alpha << ", c: " <<  c << ", p: " <<  p << ", d: " <<  d << ", q: " <<  q << "Proposal posterior:Proposal posterior:Proposal posterior:Proposal posterior: " <<  newposterior <<  ", using newd/newq: " << newd << ", " << newq <<  ", comparison to old posterior: " << exp(newposterior-currposterior) << " for MCMC sample " << i << " in post draw " << s << endl;
      // newposterior = SThBranchingPosteriorMS(ts,marks,z, Hevals,kappaevals, Gevals, newc,newp);
    ...
    // Generate a draw from the (K, alpha) posterior using these Hevals and values
    // currposterior = STkappaBranchingPosteriorMS(ts,marks,numtriggered,M0, Hevals, Gevals, logK,alpha);
    posteriorKalphaCurr = STkappaBranchingPosteriorMS(ts,marks,numtriggered,M0, Hevals, Gevals, KCurr, alphaCurr);
    ...
    for (int i=0; i < numMCMCSamples; i++) {
      ...
      // newlogK = logK + rnorm_K(gen);
      // newalpha = alpha + rnorm_K(gen);
      logKNew = logKCurr + rnorm_K(gen); // Free K
      ...
      // newposterior = STkappaBranchingPosteriorMS(ts,marks,numtriggered,M0, Hevals, Gevals, newlogK,newalpha);
      posteriorKalphaNew = STkappaBranchingPosteriorMS(ts,marks,numtriggered,M0, Hevals, Gevals, KNew, alphaNew);
...
// [[Rcpp::export]]
List STestimateETASBranchingFixBMSList(std::vector<double> &ts, std::vector<double> &marks,
                                              std::vector<double> &lons, std::vector<double> &lats, 
                                              std::vector<int> &branching, double maxT, double M0, 
                                              int sims, int numMCMCSamples, 
                                              double mu, double logK,
  ...
  for (int s=0; s<sims; s++) {
    ...
    // Run the STsampleBranchingMS fn with the current param values
    // STsampleBranchingMS(ts, lons, lats, marks, M0, muCurr,KCurr, alphaCurr, cCurr, 
                        // pCurr, dCurr, qCurr);
    ...
    // mu = rgamma(gen)/pow(1000,2) * (pow(500,2)/pow(1000,2)); // 7/31
    double mutempCurr = rgamma(gen); // 8/3
    muCurr = mutempCurr/pow(1000,2);
    // mu = rgamma(gen);
    // mu = 0.1/(pow(1000,2) * 4); // fix mu to truth
    mus.push_back(muCurr);
    // Rcout <<  "mutemp is " << mutemp <<  "mu is " << mu << endl;
    ...
    for (int i=0; i < numMCMCSamples; i++) {
      ...
      KNew = exp(logKNew);
      alphaNew = alphaCurr + rnorm_K(gen);
...
// TODO:                                 // NumericMatrix *cat_Gevals_grid as input argument
// extern "C" {
//   //if approx==1 then use infinite time approximation
//   void STestimateETASBranchingC(double *ts, double *marks, double *lons, double *lats, int *branching,  int *n, 
//                                 double *maxT, double *M0, int *sims, int *numMCMCSamples, 
//                                 int *approx, double *mu, double *logK, double *alpha, double *c, 
//                                 double *p, double *d, double *q, 
//                                 NumericMatrix &cat_Gevals_grid,
//                                 double *mus, double *logKs, double *alphas, double *cs, 
//                                 // I'm not sure if cat_Gevals_grid should be double - its a 2D vector of doubles
//                                 double *ps, double *ds, double *qs) {
//     
//     // Rcout << sims << endl;
//     
//     std::vector<double> vts(ts,ts + *n);
//     std::vector<double> vlons(lons,lons + *n);
//     std::vector<double> vlats(lats,lats + *n);
...
//     
//     Rcout <<  "hi" << endl;
//     Rcout <<  cat_Gevals_grid(9,5) << endl;
//     // estimateETASBranchingInteractionMS(vts, vmarks, vbranching, *maxT, *M0,*sims, *numMCMCSamples, *mu, *logK, *alpha, *c, *p, vmus, vlogKs, valphas, vcs, vps);
//     // STestimateETASBranchingInteractionMS(vts, vmarks, vlons, vlats, vbranching,*maxT, *M0, *sims,
//     //                                      *numMCMCSamples, 
//     //                                      *mu, *logK, *alpha, *c, *p, *d, *q, *cat_Gevals_grid,
//     //                                      vmus, vlogKs,
//     //                                      valphas, vcs, vps, vds, vqs);
//     // Rcout <<  n << endl;
//     
//     Rcout <<  "bye" << endl;
//     
...
//     std::copy(vqs.begin(),vqs.end(),qs);
//     
//     // Copy vbranching (updated in the posterior sampling function) to the branching object
//     // TODO: Do I want this?
//     // std::copy(vbranching.begin(), vbranching.end(), branching);
//     
//     return;
//   }
// }
